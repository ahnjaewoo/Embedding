...
int main(int argc, char* argv[]){
	...
	if (data_root_id == 0){
		...
		if (train_model == 0) {
			model = new TransE(Dbpedia, LinkPredictionTail, report_path, dim, alpha, training_threshold, true, worker_num, master_epoch, worker_sock, fs_log, precision);
		} else if (train_model == 1) {
			model = new TransG(Dbpedia, LinkPredictionTail, report_path, dim, alpha, training_threshold, n_cluster, crp, 10, false, true, true, worker_num, master_epoch, worker_sock, fs_log, precision);
		} else {
			cout << "[error] embedding > training model mismatch, recieved: " << train_model << endl;
			cout << "[error] embedding > return -1\n";
			fprintf(fs_log, "[error] embedding > training model mismatch, recieved : %d\n", train_model);
