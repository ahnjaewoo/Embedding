...
int main(int argc, char* argv[])
{
	...
	for(int temp = 0; temp < 5; temp++){
		...
		if ((embedding_sock[temp] = socket(PF_INET, SOCK_STREAM, 0)) < 0){
			...
			printf("[error] embedding.cpp > create socket\n");
			return -1;
		}

		if (bind(embedding_sock[temp], (struct sockaddr *)&embedding_addr, sizeof(embedding_addr)) < 0){

			printf("[error] embedding.cpp > bind socket\n");
			return -1;
		}

		if (listen(embedding_sock[temp], 1) < 0){

			printf("[error] embedding.cpp > listen socket\n");
			return -1;
		}
	}
	...
	while (1){
		...
		if ((worker_sock = accept(embedding_sock[epoch % 5], (struct sockaddr *)&worker_addr, &len)) < 0){
			...
			printf("[error] embedding.cpp > accept socket\n");
			return -1;
		}
		else{

			printf("[error] embedding.cpp > accept socket successfully\n");
		}

		if (recv(worker_sock, &flag_iter, sizeof(flag_iter), 0) < 0){

			printf("[error] embedding.cpp > recv flag_iter\n");
			close(worker_sock);
			break;
		}
		...
		if (ntohl(flag_iter) == 1){
			...
			printf("[info] embedding.cpp > recv quit signal (flag_iter = 1), quit");
			close(worker_sock);
			break;
		}
		...
		if(recv(worker_sock, &worker_num, sizeof(worker_num), 0) < 0){
			...
			printf("[error] embedding.cpp > recv worker_num\n");
			close(worker_sock);
			break;
		}
		...
		if(recv(worker_sock, &master_epoch, sizeof(master_epoch), 0) < 0){
			...
			printf("[error] embedding.cpp > recv master_epoch\n");
			close(worker_sock);
			break;
		}
		...
		if(recv(worker_sock, &dim, sizeof(dim), 0) < 0){
			...
			printf("[error] embedding.cpp > recv dim\n");
			close(worker_sock);
			break;
		}
		...
		if(recv(worker_sock, &alpha, sizeof(alpha), 0) < 0){
			...
			printf("[error] embedding.cpp > recv alpha\n");
			close(worker_sock);
			break;
		}
		...
		if(recv(worker_sock, &training_threshold, sizeof(training_threshold), 0) < 0){
			...
			printf("[error] embedding.cpp > recv training_threshold\n");
			close(worker_sock);
			break;
		}
		...
		if(recv(worker_sock, &data_root_id, sizeof(data_root_id), 0) < 0){
			...
			printf("[error] embedding.cpp > recv data_root_id\n");
			close(worker_sock);
			break;
		}
		...
		printf("[info] embedding.cpp > successfully recv params\n");
		...
		*/
		...
		{
			printf("[error] embedding.cpp > recv data root id\n");
		}
