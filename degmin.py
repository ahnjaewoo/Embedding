...
try:
    while True:
        ...
        # high-degree vertices를 정렬한 후에 anchor_num 만큼을 뽑아버린다.
        sorted_entity_degree = defaultdict(int)
        for vertex in entities_id.difference(old_anchor):
            
            sorted_entity_degree[vertex] = entity_degree[vertex]
        
        sorted_entity_degree = sorted(non_anchor_entity_degree.iteritems(), key=lambda (k,v):v,reverse=True)

        if len(sorted_entity_degree) < anchor_num:

            printt('[error] degmin > no vertex added to anchor')

        anchor = set([li[0] for li in sorted_entity_degree][:anchor_num])
